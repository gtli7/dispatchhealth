    ...
    WITH z AS (
      SELECT
         ...
         SELECT
          ...
          pp.position,
          SUM(((EXTRACT(EPOCH FROM st.end_time) - EXTRACT(EPOCH FROM st.start_time)) / 3600)) AS scheduled_hours
          ...
          SELECT
            ...
            s.scheduled_hours,
          CASE
            WHEN z.direct_clinical_hours >= 15 THEN z.direct_clinical_hours / z.count_shift_names
            ELSE z.direct_clinical_hours
          END AS punched_clinical_hours,
          CASE
            WHEN s.scheduled_hours < 0.25  AND z.direct_clinical_hours IS NULL THEN NULL
            ELSE COALESCE(z.direct_clinical_hours, s.scheduled_hours, NULL)
          ...
          FROM s
          LEFT JOIN z
              ON s.user_id = z.employee_id AND DATE(s.shift_date) = z.counter_date
          WHERE s.position IN ('emt','advanced practice provider')
      ...
      indexes: ["shift_date", "user_id"]
    }

    dimension: primary_key {
      type: number
      primary_key: yes
      sql: CONCAT(${shift_date_date}, ${user_id}) ;;
    }
