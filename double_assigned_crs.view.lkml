view: double_assigned_crs {
  derived_table: {
    sql: select care_request_id, count_shifts
from
(select care_request_id, count(distinct cs_shift_team_id) as count_shifts
from
(select
...
group by 1,2,3,4,5,6)lq
group by 1)llq
where count_shifts>1
